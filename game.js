...
document.getElementById("loop-button").addEventListener("click", function () {
  ...
  if (loopCount >= maxLoops) {
    ...
    document.getElementById("loop-counter").style.display = "none";
    document.getElementById("winning-message").style.display = "block";
    document.getElementById("next-level-1-div").style.display = "block";
  }
});
...
shapes.forEach((shape) => {
  shape.addEventListener("click", function () {
    ...
    if (currentSubLevel === 1 && chosenColor === "red") {
      ...
      currentSubLevel === 2 &&
      (chosenColor === "blue" || chosenColor === "square")
    ) {
      correct = true; // Sub-Level 2: Click if the shape is blue or a square
    } else if (
      currentSubLevel === 3 &&
      chosenColor === "red" &&
      this.classList.contains("square")
    ) {
      correct = true;
      clearedSubLevel3 = true; // Sub-Level 3: Click if the shape is red and a square
    }
    ...
    if (currentSubLevel === maxSubLevels && clearedSubLevel3) {
      document.getElementById("next-sub-level").style.display = "none"; // Hide button
...
document
  .getElementById("next-sub-level")
  .addEventListener("click", function () {
    if (currentSubLevel < maxSubLevels) {
      // Hide the current sub-level and move to the next one
      document.getElementById(`sub-level-${currentSubLevel}`).style.display =
        "none";
      currentSubLevel++;
      document.getElementById(`sub-level-${currentSubLevel}`).style.display =
        "block";
